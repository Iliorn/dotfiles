...
# mtui (BluOS music TUI) is retired — not used anymore. Remove any copy a
# previous run of this script installed.
cleanup_retired_mtui() {
    local bin="$HOME/.local/bin/mtui"
    if [[ -f "$bin" ]]; then
        log "Removing retired mtui binary"
        run rm "$bin"
    fi
}
...
# Every host in this repo uses iwd for wifi (see PACMAN_PACKAGES); NetworkManager
# is never intentionally wanted. If it's present (e.g. leftover from a distro
# default install), disable it so it can't fight iwd over the wifi device.
disable_networkmanager() {
    if skipped network; then log "Skipping NetworkManager disable"; return; fi
    if ! systemctl list-unit-files NetworkManager.service >/dev/null 2>&1; then
        ok "NetworkManager not installed"
        return
    fi
    if systemctl is-enabled --quiet NetworkManager 2>/dev/null \
        || systemctl is-active --quiet NetworkManager 2>/dev/null; then
        log "Disabling NetworkManager (this host uses iwd)"
        run sudo systemctl disable --now NetworkManager
    else
        ok "NetworkManager already disabled"
    fi
}

# When NetworkManager has ever run on a host it leaves a static, unmanaged
# /etc/resolv.conf ("# Generated by NetworkManager") behind. Once NM is gone
# nothing updates that file anymore, and its presence makes tailscaled log a
# false "systemd-resolved and NetworkManager are wired together incorrectly"
# health warning. Point it at systemd-resolved's own stub instead.
fix_resolv_conf() {
    if skipped network; then log "Skipping resolv.conf fix"; return; fi
    local stub="/run/systemd/resolve/stub-resolv.conf"
    if [[ -L /etc/resolv.conf ]] && [[ "$(readlink /etc/resolv.conf)" == "$stub" ]]; then
        ok "/etc/resolv.conf already symlinked to systemd-resolved stub"
        return
    fi
    log "Pointing /etc/resolv.conf at systemd-resolved's stub"
    [[ -e /etc/resolv.conf ]] && run sudo cp /etc/resolv.conf /etc/resolv.conf.bak-nm
